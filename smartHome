...
void ConfigureAdc_ExternalTemp();
...
void main(void)
{
    ...
    while(1){
        ...
        if (counter == 0)
        {
                ...
                _delay_cycles(20000000);            // delay for about 1 second so LED doesn't flash too fast
                externalTemp = ADC10MEM;                 // read the converted data into a variable
                P1OUT &= ~BIT6;                     // green LED off
                ADC10CTL0 &= ~ADC10IFG;             // clear the flag
                itoa(((externalTemp* 48724 - 30634388) >> 16),result,10); // convert temperature reading to Fahrenheit
        ...
        if (counter == 1)
        {
            ...
            _delay_cycles(20000000);              // delay for about 1 second so LED doesn't flash too fast
            _delay_cycles(20000000);            // delay for about 1 second so LED doesn't flash too fast
            motion = ADC10MEM;                 // read the converted data into a variable
            P1OUT &= ~BIT6;                     // green LED off
            ADC10CTL0 &= ~ADC10IFG;             // clear the flag
            itoa(motion,result,10); // convert temperature reading to Fahrenheit
...
void ConfigureAdc_ExternalTemp(){//pin 1.5
     ADC10CTL0 &= ~ENC;
...
void ConfigureAdc_Motion(){//pin 1.4
     ADC10CTL0 &= ~ENC;
...
void ConfigureAdc_Light(){//pin 1.6
     ADC10CTL0 &= ~ENC;
